...
| `update_status` | Transition status with optimistic version lock. `canceled` is terminal and withdraws any non-terminal task (`queued`/`in_progress`/`review`/`blocked`); `done`/`failed` cannot be canceled. Epic close-guard: `done`/`failed` rejected while non-terminal children exist → `CHILDREN: N sub-tasks not terminal` |
